            ...
            <div class="flex items-center gap-2">
                ...
                @if ($selectedClassId && $selectedTermId)
                    <button
                        ...
                        wire:loading.attr="disabled"
                        wire:target="clearAllSlots"
                        wire:confirm="Clear all timetable slots for this class? Locked slots will be kept. This cannot be undone."
                        class="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors bg-red-100 hover:bg-red-200 text-red-700 dark:bg-red-900/30 dark:hover:bg-red-900/50 dark:text-red-400"
                    >
                        <svg class="w-5 h-5" wire:loading.remove wire:target="clearAllSlots" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                        </svg>
                        <svg class="animate-spin w-5 h-5" fill="none" viewBox="0 0 24 24" wire:loading wire:target="clearAllSlots">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                        </svg>
                        <span wire:loading.remove wire:target="clearAllSlots">Clear All</span>
                        <span wire:loading wire:target="clearAllSlots">Clearing...</span>
                    </button>
                @endif
            </div>
                                        ...
                                        @if ($slot)
                                            ...
                                            <div class="flex items-center justify-between pt-2">
                                                @php
                                                    ...
                                                    $markerLabel = $isUnsaved ? 'UNSAVED' : 'SAVED';
                                                    $markerClass = $isUnsaved
                                                        ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400'
                                                        : 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400';
                                                @endphp
                                                <span class="px-2 py-1 text-xs font-medium rounded {{ $markerClass }}">
                                                    {{ $markerLabel }}
                                                </span>
                                            </div>
                                        </div>
